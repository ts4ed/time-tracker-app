...
import dark from '../../themes/themes/dark';
import light from '../../themes/themes/light';

const TimeTracker = () => {
  const currentItems = localStorage.getItem('elements')
    ? JSON.parse(localStorage.getItem('elements'))
    : [];

  const [item, setItem] = useState(currentItems);

  const [storedItems, setStoredItems] = useState(currentItems);
  ...
  const icon =
    theme.title === 'light' ? <HiMoon size={15} /> : <CgSun size={15} />;
    ...
    <ThemeProvider theme={theme}>
      <Layout>
        ...
        <div className="scroll-wrapper">
          <ul className="list">
            {storedItems.map(
              ({ id, title, isRunning, startingTime, finalTime }) => (
                <TrackerItem
                  key={id}
                  title={title}
                  isRunning={isRunning}
                  countdown={() =>
                    countdownUpdate(startingTime, finalTime, isRunning)
                  }
                  delTracker={() => delTracker(id)}
                  playClick={() => playClick(id)}
                />
              )
            )}
          </ul>
